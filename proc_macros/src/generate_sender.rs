...
pub fn generate_full_sender(input: TokenStream) -> TokenStream {
    let spec: FullSpec = parse_macro_input!(input);
...
impl ToTokens for FullSpec {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        let struct_name = format_ident!("{}Sender", self.enum_name);
        let enum_name = &self.enum_name;
        ...
        quote! {
            ...
            #[::async_trait::async_trait]
            impl ::oelung_lantern::mpsc::Sender<#enum_name> for #struct_name {
                async fn send(&self, value: #enum_name) {
                    self.sender.send(value).await.unwrap();
                }

                fn box_clone(&self) -> ::std::boxed::Box<dyn ::oelung_lantern::mpsc::Sender<#enum_name>> {
                    Box::new(self.clone())
                }
            }
        }
...
pub fn generate_sender_from_sender(input: TokenStream) -> TokenStream {
    let spec: FromSenderSpec = parse_macro_input!(input);
...
struct FromSenderSpec {
    pub enum_name: Ident,
    pub variant_name: Ident,
    pub type_: Path,
}

impl Parse for FromSenderSpec {
    fn parse(input: ParseStream) -> Result<Self> {
        let enum_name: Ident = input.parse()?;
        input.parse::<Token![,]>()?;
        let variant_name: Ident = input.parse()?;
...
impl ToTokens for FromSenderSpec {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        let struct_name = format_ident!("{}Sender", self.variant_name);
        let type_ = &self.type_;
        let enum_name = &self.enum_name;
        ...
        quote! {
            struct #struct_name {
                pub sender: ::std::boxed::Box<dyn ::oelung_lantern::mpsc::Sender<#enum_name>>,
            }
            ...
            impl From<::std::boxed::Box<dyn ::oelung_lantern::mpsc::Sender<#enum_name>>> for #struct_name {
                fn from(value: ::std::boxed::Box<dyn ::oelung_lantern::mpsc::Sender<#enum_name>>) -> Self {
                    Self {
                        sender: value,
                    }
                }
            ...
            impl ::oelung_lantern::mpsc::Sender<#type_> for #struct_name {
                async fn send(&self, value: #type_) {
                    self.sender.send(#enum_name::#variant_name(value)).await;
                }
