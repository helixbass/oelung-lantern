...
use oelung_lantern::{generate_sender, mpsc::Sender, spinner::balls, BallsSpinner, ReceiveEvent};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let mut spinner = BallsSpinner::new(
        None,
        Some(Color::Magenta),
        Box::new(BallsSpinnerTickSender::from(sender)),
    );
...
fn render_screen(renderer: &mut Renderer, spinner: &BallsSpinner) -> Result<(), anyhow::Error> {
...
enum World {
    Crossterm(Event),
    BallsSpinnerTick(balls::Tick),
}

generate_sender!(World, Crossterm, Event);
generate_sender!(World, BallsSpinnerTick, balls::Tick);
