...
use oelung_lantern::{generate_sender, mpsc::Sender, spinner::bounce, BounceSpinner, ReceiveEvent};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let mut spinner = BounceSpinner::new(
        None,
        Some(Color::Yellow),
...
fn render_screen(renderer: &mut Renderer, spinner: &BounceSpinner) -> Result<(), anyhow::Error> {
...
enum World {
    Crossterm(Event),
    BounceSpinnerTick(bounce::Tick),
}

generate_sender!(World, Crossterm, Event);
generate_sender!(World, BounceSpinnerTick, bounce::Tick);
