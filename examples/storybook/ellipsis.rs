...
use oelung_lantern::{
    mpsc::Sender,
    spinner::{self, ellipsis},
    storybook, ReceiveEvent,
};

use super::{EllipsisSpinnerTickSender, World};

pub struct EllipsisSpinner {}

impl EllipsisSpinner {
    pub fn new() -> Self {
        Self {}
    }
}

impl storybook::Component<World> for EllipsisSpinner {
    fn name(&self) -> SmolStr {
        "Ellipsis spinner".to_smolstr()
    }
    ...
    fn get_component(
        ...
        Box::new(spinner::EllipsisSpinner::new(
            Some(period.clone()),
            Some(color.clone()),
...
impl storybook::ComponentInstance<World> for spinner::EllipsisSpinner {
    ...
    fn receive<'a>(
        ...
        Ok(match event {
            World::EllipsisSpinnerTick(tick) => {
                ReceiveEvent::<ellipsis::Tick>::receive(self, tick, &mut queue_effect)?;
            }
            _ => {}
        })
    }
