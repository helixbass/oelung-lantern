...
use super::{World, WorldSpinnerTickSender};

pub struct WorldSpinner {}
...
impl storybook::Component<World> for WorldSpinner {
    fn name(&self) -> SmolStr {
        "World spinner".to_smolstr()
    }
    ...
    fn get_component(
        ...
        Box::new(spinner::WorldSpinner::new(
            Some(period.clone()),
            Box::new(WorldSpinnerTickSender::from(sender)),
        ))
    }
}

impl storybook::ComponentInstance<World> for spinner::WorldSpinner {
    ...
    fn receive<'a>(
        ...
        Ok(match event {
            World::WorldSpinnerTick(tick) => {
                ReceiveEvent::<world::Tick>::receive(self, tick, &mut queue_effect)?;
            }
            _ => {}
        })
    }
