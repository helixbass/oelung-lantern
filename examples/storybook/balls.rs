...
use super::{BallsSpinnerTickSender, World};
...
impl storybook::Component<World> for BallsSpinner {
    fn name(&self) -> SmolStr {
        "Balls spinner".to_smolstr()
    }
    ...
    fn get_component(
        ...
        Box::new(spinner::BallsSpinner::new(
            Some(period.clone()),
            Some(color.clone()),
...
impl storybook::ComponentInstance<World> for spinner::BallsSpinner {
    ...
    fn receive<'a>(
        ...
        Ok(match event {
            World::BallsSpinnerTick(tick) => {
                ReceiveEvent::<balls::Tick>::receive(self, tick, &mut queue_effect)?;
            }
            _ => {}
        })
    }
