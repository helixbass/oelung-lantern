...
use super::{MonkeySpinnerTickSender, World};
...
impl storybook::Component<World> for MonkeySpinner {
    fn name(&self) -> SmolStr {
        "Monkey spinner".to_smolstr()
    }
    ...
    fn inputs(&self) -> Vec<Rc<storybook::Input>> {
        vec![Rc::new(storybook::Input::new(
            ...
            storybook::InputValue::Duration(Duration::from_millis(monkey::DEFAULT_PERIOD)),
        ))]
    }
    ...
    fn get_component(
        ...
        Box::new(spinner::MonkeySpinner::new(
            Some(period.clone()),
            Box::new(MonkeySpinnerTickSender::from(sender)),
        ))
    }
}

impl storybook::ComponentInstance<World> for spinner::MonkeySpinner {
    ...
    fn receive<'a>(
        ...
        Ok(match event {
            World::MonkeySpinnerTick(tick) => {
                ReceiveEvent::<monkey::Tick>::receive(self, tick, &mut queue_effect)?;
            }
            _ => {}
        })
    }
