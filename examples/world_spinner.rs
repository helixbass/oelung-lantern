...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let mut spinner = WorldSpinner::new(None, Box::new(WorldSpinnerTickSender::from(sender)));
...
fn render_screen(renderer: &mut Renderer, spinner: &WorldSpinner) -> Result<(), anyhow::Error> {
...
enum World {
    Crossterm(Event),
    WorldSpinnerTick(world::Tick),
}

generate_sender!(World, Crossterm, Event);
generate_sender!(World, WorldSpinnerTick, world::Tick);
