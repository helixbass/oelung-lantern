...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let mut spinner = MonkeySpinner::new(None, Box::new(MonkeySpinnerTickSender::from(sender)));
...
fn render_screen(renderer: &mut Renderer, spinner: &MonkeySpinner) -> Result<(), anyhow::Error> {
...
enum World {
    Crossterm(Event),
    MonkeySpinnerTick(monkey::Tick),
}

generate_sender!(World, Crossterm, Event);
generate_sender!(World, MonkeySpinnerTick, monkey::Tick);
